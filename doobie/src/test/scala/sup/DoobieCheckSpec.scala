...
class DoobieCheckSpec extends BaseIOTest {
  ...
  "IO H2 check" when {
    "the database responds before the timeout" should {
      "be Healthy" in runIO {
        val healthCheck = sup.doobie.connectionCheck(goodTransactor[IO])(timeoutSeconds = Some(5))
    ...
    "there is no timeout" should {
      "be Healthy" in runIO {
        val healthCheck = sup.doobie.connectionCheck(goodTransactor[IO])(timeoutSeconds = None)
